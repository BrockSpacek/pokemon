...
import { getLocalStorage } from "@/lib/DataServices";
...
export default function Home() {
  ...
  function tryParseInt(str: string) {
    ...
    return isNaN(parsedValue) ? str : parsedValue;
  }

  const handleSearchClick = () => {
    if (!input) return;

    const searchValue: number | string = tryParseInt(input);

    if ( typeof searchValue === "number" && (searchValue < 1 || searchValue > 649) ) {
